      ...
      $id=intval($_GET['id']);
           ...
           if($i=mysqli_num_rows($result)){
              $view_errors.= "This problem is in Contest(s) below:<br>";
                   for (;$i>0;$i--){
                                $row=mysqli_fetch_row($result);
                                $view_errors.= "<a href='problem.php?cid=".$row[0]."&pid=".$row[2]."&lang=".$OJ_LANG.">Contest $row[0]:$row[1]</a><br>";
...
foreach($row as $key=>$value)
{
    ...
    //echo $value."\n";
   // echo $value."\n";
    $json_data[$key]=preg_replace('/<[\s\S]+?>/','',$value);
    $json_data[$key]=preg_replace('/[\s\S]+?}/','',$json_data[$key]);
    $json_data[$key]=str_replace("<p>","",$json_data[$key]);
    $json_data[$key]=str_replace("</p>","",$json_data[$key]);
    $json_data[$key]=str_replace("<br />","\n",$json_data[$key]);
    $json_data[$key]=str_replace("&nbsp;"," ",$json_data[$key]);
    $json_data[$key]=str_replace("&quot;","\"",$json_data[$key]);
    $json_data[$key]=str_replace("&lt;","<",$json_data[$key]);
    $json_data[$key]=str_replace("&gt;",">",$json_data[$key]);
    $json_data[$key]=str_replace("&amp;","&",$json_data[$key]);
    $json_data[$key]=str_replace("&le;","≤",$json_data[$key]);
   // echo "\ntest\n".$json_data[$key]."\ntest\n";
}
unset($value);
...
//$json_data=array('id'=>$row->problem_id,'title'=>$row->title,'time-limit'=>$row->time_limit,'memory-limit'=>$row->memory-limit,'submit'=>$row->submit,'accepted'=>$row->accepted,'description'=>$row->description,'input'=>$row->input,'output'=>$row->output,'sample-input'=>$row->sample_input,'sample-output'=>$row->sample_output,'hint'=>$row->hint,'source'=>$row->source);
