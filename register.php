...
$len=strlen($confirmquestion);
if($len==0)
{
    $err_str=$err_str."Confirm Question Too short!\\n";
    $err_cnt++;
}
$len=strlen($confirmanswer);
if($len==0)
{
    $err_str=$err_str."Confirm Answer Too short!\\n";
    $err_cnt++;
}
if ($err_cnt>0){
...
$result=$database->count("users",["user_id"=>$user_id]);
if($result>0)
{
    print "<script language='javascript'>\n";
...
."`user_id`,`email`,`ip`,`accesstime`,`password`,`reg_time`,`nick`,`school`,`confirmquestion`,`confirmanswer`)"
."VALUES('".$user_id."','".$email."','".$_SERVER['REMOTE_ADDR']."',NOW(),'".$password."',NOW(),'".$nick."','".$school."','".$confirmquestion."','".$confirmanswer."')";
