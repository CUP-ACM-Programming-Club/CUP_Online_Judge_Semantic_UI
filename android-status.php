...
if($OJ_MEMCACHE)
        ...
        $row=mysqli_fetch_array($result);
        array_push($total_row,$row);
        /*foreach($row as $key=>$value)
        {
            $total_row[$rows_cnt][$key]=$value;
        }*/
        //$view_status[$i]=$row;
        ...
        if ($row['contest_id']>0) {
                $view_status[$i][1]= "<a href='contestrank.php?cid=".$row['contest_id']."&lang=".$OJ_LANG."&user_id=".$row['user_id']."#".$row['user_id']."'>".$row['user_id']."</a>";
        }else{
                $view_status[$i][1]= "<a href='userinfo.php?user=".$row['user_id']."&lang=".$OJ_LANG."'>".$row['user_id']."</a>";
        }

       if ($row['contest_id']>0) {
                $view_status[$i][2]= "<div class=center><a href='problem.php?cid=".$row['contest_id']."&lang=".$OJ_LANG."&pid=".$row['num']."'>";
                if(isset($cid)){
                ...
                $view_status[$i][2]= "<div class=center><a href='problem.php?id=".$row['problem_id']."&lang=".$OJ_LANG."'>".$row['problem_id']."</a></div>";
        }
	...
	$view_status[$i][3]="";
        if (intval($row['result'])==11 && ((isset($_SESSION['user_id'])&&$row['user_id']==$_SESSION['user_id']) || isset($_SESSION['source_browser']))){
                $view_status[$i][3].= "<a href='ceinfo.php?sid=".$row['solution_id']."&lang=".$OJ_LANG."' class='".$judge_color[$row['result']]."'  title='$MSG_Click_Detail'>".$MSG_Compile_Error."</a>";
                ...
                $view_status[$i][3].= "<a href='reinfo.php?sid=".$row['solution_id']."&lang=".$OJ_LANG."' class='".$judge_color[$row['result']]."' title='$MSG_Click_Detail'>".$judge_result[$row['result']]."</a>";
              ...
              if(!$lock||$lock_time>$row['in_date']||$row['user_id']==$_SESSION['user_id']){
                if($OJ_SIM&&$row['sim']>80&&$row['sim_s_id']!=$row['s_id']) {
                        ...
                        if( isset($_SESSION['source_browser'])){
                                        ...
                                        $view_status[$i][3].= "<a href=comparesource.php?left=".$row['sim_s_id']."&right=".$row['solution_id']."&lang=".$OJ_LANG."  class='btn btn-info'  target=original>".$row['sim_s_id']."(".$row['sim']."%)</a>";
                        }else{
        ...
        if ($flag){
                ...
                if (!(isset($_SESSION['user_id'])&&strtolower($row['user_id'])==strtolower($_SESSION['user_id']) || isset($_SESSION['source_browser']))){
                        ...
                        $view_status[$i][6]= "<a target=_blank href=showsource.php?id=".$row['solution_id']."&lang=".$OJ_LANG.">".$language_name[$row['language']]."</a>";
			if($row["problem_id"]>0){
                        	if (isset($cid)) {
                                	$view_status[$i][6].= "/<a target=_self href=\"submitpage.php?cid=".$cid."&pid=".$row['num']."&lang=".$OJ_LANG."&sid=".$row['solution_id']."\">Edit</a>";
                        	}else{
                                	$view_status[$i][6].= "/<a target=_self href=\"submitpage.php?id=".$row['problem_id']."&lang=".$OJ_LANG."&sid=".$row['solution_id']."\">Edit</a>";
                        	}
			}
                }
