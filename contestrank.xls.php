...
$sql="SELECT 
	users.user_id,users.nick,solution.result,solution.num,solution.in_date 
		FROM 
			(select user_id,result,num,in_date,contest_id from solution where contest_id='$cid' and num>=0 and problem_id>0 union all select user_id,result,num,in_date,contest_id from vjudge_solution where contest_id='$cid' and num>=0 and problem_id>0) solution 
		left join users 
...
echo "<table border=1><tr><td>Rank<td>User<td>Nick<td>Solved<td>Time<td>Mark";
for ($i=0;$i<$pid_cnt;$i++)
	echo "<td>$PID[$i]";
echo "</tr>";
...
for ($i=0;$i<$user_cnt;$i++){
	...
	echo "<td>".$U[$i]->nick."";
    /*$result=$database->select("users","*",["user_id"=>intval(trim($U[$i]->nick))]);
		$U[$user_cnt]->name=$result[0]["nick"];
	echo "<td>".$result[0]["nick"];
	*/
	echo "<td>$usolved";
	$hour=0;
	$minute=0;
	$second=$U[$i]->time%60;
	$hour=intval($U[$i]->time/3600);
	$minute=$U[$i]->time-$hour*60*60;
	$minute=intval($minute/60);
	echo "<td>$hour:$minute:$second";
	echo "<td>";
