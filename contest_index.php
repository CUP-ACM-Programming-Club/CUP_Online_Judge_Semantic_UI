...
<!--<h4></h4>-->
            <a class="ui huge inverted download button" href="cprogrammingcontest.php" target="_blank">查看考场信息</a>
             <a class="ui huge inverted download button" target="_blank" href='update_log.php'>更新日志</a>
        <h4></h4>
        <div class="ui huge primary button" onclick="location.href='contest.php?my'">前往竞赛/考试<i class="right arrow icon"></i></div>
</div>
    </div>
...
<script>
    ...
    $(document).ready(function () {
        window.picid = 'bg' + getRandomIntInclusive(0, 13);
        $("#background")
            .addClass(picid)
            .removeClass('zoomed')
        ;
    ...
    $msg=$result[0]['msg'];
    ?>
    $('.maintain').html("最近更新维护时间:<?=$time?> <a href='update_log.php'>更新日志</a>").attr("data-html", "<div class='header'>"+"升级维护内容"+"</div><div class='content'><?=str_replace("&gt;",">",str_replace("&lt;","<",htmlentities($msg,ENT_COMPAT)))?></div>")
        .popup({
            position: 'top center',
            on: 'hover'
        })
